...
import { LayoutDashboard, FileText, LogOut, Menu, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export default function WriterLayout() {
  ...
  const [minimized, setMinimized] = useState(() => localStorage.getItem('writer-sidebar-minimized') === 'true');
  const location = useLocation();

  useEffect(() => {
    localStorage.setItem('writer-sidebar-minimized', minimized);
  }, [minimized]);
      ...
      <div className="min-h-screen bg-dark flex">
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 z-50 bg-white/[0.02] border-r border-white/5 flex flex-col transition-all duration-300 ease-in-out ${minimized ? 'lg:w-[68px]' : 'lg:w-64'} ${sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full lg:translate-x-0 w-64'}`}>
          <div className={`border-b border-white/5 flex items-center ${minimized ? 'lg:justify-center lg:p-4' : 'p-6'}`}>
            ...
            {!minimized && <p className="text-xs text-white/40 mt-1 ml-0 hidden lg:block">Writer Panel</p>}
            {!minimized && <p className="text-xs text-white/40 mt-1 lg:hidden">Writer Panel</p>}
          </div>
