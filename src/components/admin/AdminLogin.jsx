...
export default function AdminLogin() {
  ...
  useState(() => {
    fetch('/api/auth/me', { credentials: 'include' })
      ...
      .then((data) => { if (data.user?.role === 'admin') setIsAuthed(true); setChecking(false); })
  ...
  if (isAuthed) return <Navigate to="/blog/admin" replace />;

  return (
    <div className="min-h-screen bg-dark flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-white">Vioneta Admin</h1>
          <p className="text-sm text-white/40 mt-2">Sign in to the admin panel</p>
        </div>
